"""Generate statewide results of OpenET vs DNRC consumptive use methodologies.

Creates and populates a sqlite database with results for each field in the Statewide
Irrigation Dataset.
...
def init_db_tables(con):
    """Initialize the tables in the project sqlite database.
...
def update_irrmapper_table(con, file):
    ...
    Args:
        ...
        file: path to irrmapper csv file
    """
    cur = con.cursor()
...
def update_etof_db(con, etof_dir, etof_tb):
    ...
    any duplicates from being loaded. This function is insenstitive to naming schemes, as long as 
    the data is in the proper format.
...
# need to generalize this function before use
def update_etof_db_more(con, etof_dir, etof_tb):
    """Update the sqlite database table that stores OpenET etof data (later additions).

    After checking which FIPS county codes (of form "000") are already contained in the db table,
    all the data from files in etof_dir with new county codes are appended to the existing table.
    This function is highly sensitive to naming schemes, as it gets county codes from a pre-determined
    location in the filenames.

    Args:
        con: sqlite database connection
        etof_dir: path to directory where etof files are located; etof files should be
          like "ensemble_monthly_etof_000_YYYY_YYYY.csv"
        etof_tb: str, name of table in sqlite database containing etof data
    """
    ...
    # FIPS code location is counted from end of filename.
    ...
    # if any are new,
    if len(counties) > 0:
        # load in each csv file assocaited with a new county
...
def gridmet_match_db(con: sqlite3.Connection,
                     fields: gpd.GeoDataFrame | str,
                     gridmet_points: str,
                     fields_join: str) -> None:
    """Match each field centroid with closest GridMET pixel centroid and update sqlite database table.
    ...
    there may be many fields that only need data from one gridmet cell. This prevents us from
    downloading many redundant data sets. Looks like it often works out to roughly 1 GridMET
    data pull for every 10 fields in the SID.

    Args:
        con: sqlite database connection
        fields: geopandas geodataframe or filepath, shapefile containing field geometries and unique
          field ids with county identifier
    ...
    if type(fields) != gpd.GeoDataFrame:
    ...
    if ~existing_fields['fid'].isin(fields['FID']).any().any():
        ...
        print('Found {} gridmet target points for {} new fields'
              .format(len(fields['gfid'].unique()), fields.shape[0]))
        print()
    else:
...
def corrected_gridmet_db(con, gridmet_points, fields_join, gridmet_tb, gridmet_ras,
    ...
    The reason we're not just doing a zonal stat on correction surface for every object is that
    there may be many fields that only need data from one gridmet cell. This prevents us from
    downloading many redundant data sets.

    Date search function prevents creation of duplicate rows, and ensures continuous record from
    theearliest start date entered to the latest end date entered. No gaps in data are permitted.
    This may result in downloading "extra" data if periods of interest are not overlapping, but greatly
    ...
    Args:
        ...
        gridmet_ras: filepath, directory containing .tif files with gridmet correction factors,
          one for each month of year and each variable (etr and eto, 24 total files). Correction
          is to limit bias between gridmet and agrimet stations.
        ...
        selection: optional, pandas dataframe containing a column of gfids for which to load data.
          If None, function will update for all gfids contained in the database's field id/gfid
          lookup table (fields_join).
    """
    if selection.empty:
    ...
    if not target_gfids.empty:
        ...
        print(f'Get gridmet for {len(gridmet_targets)} target points')
        gridmet_pts.index = gridmet_pts['GFID']
        ...
        for index, row in tqdm(target_gfids.iterrows(), total=len(target_gfids)):
            ...
            for _var in ['etr', 'eto']:
                ...
                for month in range(1, 13):
                    ...
                    df.loc[idx, f'{variable}_uncorr'] = df.loc[idx, variable]
                    df.loc[idx, variable] = df.loc[idx, f'{variable}_uncorr'] * corr_factor

            df['tmax_c'] = df.tmax_k - 273.15
            df['tmin_c'] = df.tmin_k - 273.15

            df = df[COLUMN_ORDER]  # I don't think order actually matters, as long as names match db
            df['gfid'] = row['gfid']
...
# Does this one work? I think it's more efficient, using sql queries instead of loading
# lots of things in memory with pandas.
# But I don't know how to incorporate selection here.
def corrected_gridmet_db_1(con, gridmet_points, fields_join, gridmet_tb, gridmet_ras,
    ...
    Date search function prevents creation of duplicate rows, and ensures continuous record from 
    the earliest start date entered to the latest end date entered. No gaps in data are permitted. 
    This may result in downloading "extra" data if periods of interest are not overlapping, but greatly 
    simplifies the logic of the algorithm.
    ...
    # get list of gfids not present in timeseries result table
    target_gfids = []
    new_gfids = cur.execute(f"SELECT gfid FROM {fields_join} EXCEPT SELECT gfid FROM {gridmet_tb}")
    for idx in new_gfids:
        target_gfids.append((idx[0], start, end))

    # check for missing dates for gfids already present in timeseries result table
    old_gfids = cur.execute(f"SELECT gfid FROM {fields_join} INTERSECT SELECT gfid FROM {gridmet_tb}")
    for idx in old_gfids:
        s1 = pd.to_datetime(cur.execute(f"SELECT min(date) FROM {gridmet_tb} WHERE gfid={idx[0]}").fetchone())
        e1 = pd.to_datetime(cur.execute(f"SELECT max(date) FROM {gridmet_tb} WHERE gfid={idx[0]}").fetchone())
        if s2 >= s1 and e2 <= e1:
            continue
        if s2 < s1:
            new_e = s1 - timedelta(days=1)
    ...
    # save combinations of gfids and missing time periods 
    target_gfids = pd.DataFrame(target_gfids, columns=['gfid', 'start', 'end'])
...
def more_gridmet_vars(con, variables, gridmet_points, fields_join, gridmet_tb, gridmet_ras, start, end,
    ...
    Args:
        con: sqlite database connection
        variables: list of variables to be added to the table, each one needs to be identical
          to a column in gridmet_tb, and a 'col' value in CLIMATE_COLS_LONG
        gridmet_points: filepath, shapefile of gridmet pixel centroids
        fields_join: str, name of table in sqlite database containing field/gridmet lookup
        gridmet_tb: str, name of table in sqlite database containing gridmet data
        gridmet_ras: filepath, directory containing .tif files with gridmet correction factors,
        ...
        selection: optional, pandas dataframe containing column of gfids for which to load data.
          If None, function will update for all gfids contained in the database's field id/gfid
          lookup table (fields_join).
    """
...
def cu_analysis_db(con, shp, gridmet, etof, out, start=1985, end=2024,
    ...
    Includes both grass and alfalfa reference ET (eto and etr).
    ...
    Args:
        ...
        mf_timeperiod: int, which set of management factors to apply to dnrc_cu calculations,
          acceptable values of 0, 1, or 2 to correspond to the three time periods/set of management
          factors described in the rule.
    ...
    if len(gdf) > 0:
        ...
        for fid, yr_values in tqdm(dictionary.items(), total=len(dictionary)):
            ...
            for y in yr_values:
                ...
                fids.append(fid)
                ys.append(y)
                grd = pd.read_sql("SELECT date, eto_mm, etr_mm, tmin_c, tmax_c, prcp_mm "
                                  "FROM {} WHERE gfid={} AND year={}".format(gridmet, row[5], y), con)
                ...
                grd = grd.rename(columns={'eto_mm': 'ETOS', 'etr_mm': 'ETRS',
                                          'tmin_c': 'MN', 'tmax_c': 'MX', 'prcp_mm': 'PP'})
                ...
                grd['ETRS'] = grd['ETRS'] / 25.4  # daily ET, mm to in
        ...
        merged_data = [(fids[n], ys[n], im[n], frac_irrs[n], mf_periods[n], mfs[n], etoss[n], etrss[n],
                        etbcs[n], etofs[n], etrfs[n], opnt_cus[n], dnrc_cus[n]) for n in range(len(fids))]
        cur.executemany("INSERT INTO {} VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                        .format(out), merged_data)
        con.commit()
    else:
...
def iwr_static_cu_analysis_db(con, shp, out, clim_db_loc, mf_timeperiod=2,
                              selection=gpd.GeoDataFrame(), iwr_coord=""):
    ...
    Args:
        ...
        clim_db_loc:
        mf_timeperiod: int, which set of management factors to apply to dnrc_cu calculations,
          acceptable values of 0, 1, or 2 to correspond to the three time periods/set of
          management factors described in the rule.
        selection: GeoDataFrame, optional
        iwr_coord:
    """
    mf_list = ['1964-1973', '1973-2006', '1997-2006']
...
def pd_to_sql_ignore_on_conflict(table, conn, keys, data_iter):
    ...
    stmt = ''.join(['?, '] * len(keys))
    stmt = f'INSERT OR IGNORE INTO {table.name} VALUES({stmt[:-2]})'
...
# Old algorthim was used for current results as of 6/18/2024.
# New table must be made to use this algorithm.
def closest_iwr_station(row, jack):
    ...
    This algorithm: If there are no IWR stations in the same county as the field, the closest station
    will be chosen. Otherwise, both the closest station and the closest station within the county will
    be identified. Of the two potential stations, the one that is closer in elevation to the field will
    be chosen.
    ...
    Returns:
        Station number of the most representative IWR station.
    """
    field_county = row[4]
    field_lat = row[6]
    field_lon = row[7]
    ...
    for i, stn in jack.iterrows():
        this_dist = great_circle_distance((field_lat, field_lon), (stn['lat'], stn['lon']))
...
if __name__ == '__main__':
    ...
    # print(pd.read_sql("PRAGMA table_list", conec))
    print()
    # print(pd.read_sql("PRAGMA table_info(field_cu_results)", conec))
